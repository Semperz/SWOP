...
import com.example.swop.data.remote.api.OrderApi;
...
public class OrderRepository {
    ...
    public OrderRepository(Context ctx) { api = RetrofitClient.getInstance(ctx).create(OrderApi.class); }

    public void getAll(ApiCallback<List<OrderDto>> cb) { api.getAll().enqueue(wrap(cb, "getAll")); }
    public void myOrders(ApiCallback<List<OrderDto>> cb) { api.myOrders().enqueue(wrap(cb, "myOrders")); }
    public void getById(long id, ApiCallback<OrderDto> cb) { api.getById(id).enqueue(wrap(cb, "getById")); }
    public void create(OrderDto dto, ApiCallback<OrderDto> cb) { api.create(dto).enqueue(wrap(cb, "create")); }
    ...
    private Callback<Void> boolWrap(ApiCallback<Boolean> cb, String tag) { return new Callback<Void>() {
        ...
        @Override public void onFailure(Call<Void> call, Throwable t) { Log.e("OrderRepo", tag, t); cb.onFailure(t);} }; }
}
